...
#--------------------------------------------------------------------
# Declaration of favourite colours. 
#--------------------------------------------------------------------
...
parser._optionals.title = '**- Commands -**'
...
parser.add_argument("--trim_minimum", 
    type= int, 
    required = False,
    help="Trims reads based on a minimum quality score using PHRED 33 only.")
...
if args.cite:
    print('The citation:\nM.O.L.S, Jiawen Wu,(2022) Genvej: Read Trimmer for Next Generation Sequencing Data')
    print('version 2.0.01, https://github.com/gavinakajiawen/Read-Trimmer-for-NGS-data\n')
...
#--------------------------------------------------------------------
logging.info("Welcome to the log file")
...
if search_gz is None and search_phred33 is not None and args.trim_minimum is None:
    ...
    try:
        ...
        # raise an assertion message. 
        assert args.shear is not None and args.snip is not None and args.trim_minimum is None, "You must choose a number of nucleotides to trim from either end of the read."
        
    except TypeError as ErrorT:
        print(bright_cyan +"--shear {x} and --snip {x} commands must be used together with --filename to trim the nucleotides from the read. \n"+reset)
        sys.exit(1)
        
        # create the assertion error and give it an object name. 
    except AssertionError as errorA:
        print(bright_cyan + str(errorA) + reset)
